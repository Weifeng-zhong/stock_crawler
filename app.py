...
st.set_page_config(page_title="沪深成交数据查询", page_icon="chart", layout="centered")
...
def _sse(date_str, product_code):
    try:
        params = {"sqlId": "COMMON_SSE_SJ_GPSJ_CJGK_MRGK_C", "PRODUCT_CODE": product_code, "type": "inParams", "SEARCH_DATE": date_str}
...
with tab2:
    ...
    if st.button("批量查询", type="primary", use_container_width=True, key="batch"):
        if start_date > end_date:
            ...
            if results:
                df_yi = pd.DataFrame(results)
                df_wan = df_yi.copy()
                for col in df_wan.columns[1:]:
                    def _convert(v):
                        return f"{float(v)/10000:.4f}" if v != "-" else "-"
                    df_wan[col] = df_wan[col].apply(_convert)
                ...
                st.download_button("下载 CSV（亿元）", data=csv, file_name=f"batch_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.csv", mime="text/csv")
            else:
