...
**批量查询**
- 选择日期范围，自动筛选交易日（排除周末、法定节假日、调休上班的周末）
...
- `chinese_calendar` 库的 `is_workday()` **且非周末**（`weekday() < 5`）
- 注意：调休上班的周末（如 2026-09-20、2026-10-10）**不是交易日**，交易所周末始终休市
...
| 周期 | 事项 | 说明 |
|------|------|------|
| 每周瞄一眼 | Actions 运行状态 | 失败会发邮件通知（红叉 ❌）；展开日志看 `SSE 官方接口异常` / `获取失败` 等关键字 |
| 收到 550/535 类发送失败时 | 更新 QQ 邮箱授权码 | 重新生成后更新 GitHub Secrets 的 `MAIL_PASS` |
| 网页"推送设置"报 Token 错误时 | 更新 GITHUB_TOKEN | 在 Streamlit Cloud → Settings → Secrets 中更新（Token 有有效期） |
| 每年 10-12 月 | 无需手动操作 | `chinese_calendar` 随每次部署自动安装最新版，自带次年节假日数据 |
| 日常 | 订阅/退订 | 网页端自助操作即可 |

> 说明：GitHub 免费版定时任务存在调度延迟（常见 1-4 小时），邮件实际送达时间多为中午前后，属正常现象。
...
| 2026-09-23 | 2.8 | 修复调休周末误判交易日（9/21 失败根因）；workflow 加 rebase 防冲突 + 并发保护；网页端容错（Token 失效不崩溃、写入失败提示、全接口超时）；腾讯兜底重试；维护清单文档 |
